...
class Calculate():
    def __init__(self, model, device="cuda:0"):
        ...
        self.ckpt_saving_path = os.path.join(f"checkpoints", f"{args.model}_{pos}_{args.dec_model}_shuffle_pe")
    ...
    def train(self, config):
        ...
        if C.args.dataset == "vifi":
            self.train_dataloader, self.valid_dataloader = load_data(config, shuffle_unsup=True)
        ...
        interl_gps, nearst_gps, imu, bbx4, passersby_bbx4, passersby_interl_gps, passersby_nearst_gps, shuffle_pkg = next(iter(self.train_dataloader))
        ...
        for epoch in range(args.epochs):
            ...
            for idx, (interl_gps, nearst_gps, imu, bbx4, passersby_bbx4, passersby_interl_gps, passersby_nearst_gps, shuffle_pkg) in enumerate(self.train_dataloader): # interl_gps, nearst_gps, imu19, bbx4
                
                out_of_model = []

                for interl_gps, nearst_gps, imu, bbx4, passersby_bbx4, passersby_interl_gps, passersby_nearst_gps in shuffle_pkg:
                    ...
                    pred_bbx, denoised_bbx_observation, cam_int = self.model(interl_gps, nearst_gps, in_imu19, bbx4, passersby_bbx4, passersby_interl_gps, passersby_nearst_gps, imu, gen_cam_intr=True) # bbx is predicted sequence, trans_bbx is the denoised bbx
                    out_of_model.append([denoised_bbx_observation, bbx4_in_gt, pred_bbx, bbx4, cam_int])
                    # for baseline
                ...
                loss = 0
                cam_ints = []
                for denoised_bbx_observation, bbx4_in_gt, pred_bbx, bbx4, cam_int in out_of_model:
                    ...
                    loss = loss + loss_denoise + loss_pred
                loss_intri_consist = self.criterion(cam_ints[0], cam_ints[1]) + self.criterion(cam_ints[2], cam_ints[1])
                loss = loss + loss_intri_consist
                loss.backward()
                self.optimizer.step()

                # train_epoch_loss["loss"].append(loss.item())
                train_epoch_loss["loss"].append(loss_intri_consist.item()) ###TODO: need to change back
...
if __name__=="__main__":
    ...
    parser.add_argument("--model", nargs="?", type=str, default= "Transformer",choices=["ViTag", "LSTM", "NMT", "UNet", "Transformer","SingleLayerViTag","VisionPosition"], help="input img size")
    parser.add_argument("--dec_model", nargs="?", type=str, default= "Transformer",choices=["ViTag","GRU","RNN", "LSTM", "NMT", "UNet", "Transformer","SingleLayerViTag","VisionPosition"], help="input img size")
