...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.l2_loss = lambda a, b: (a - b) ** 2  # torch.nn.MSELoss(reduction='none')  # must be None for handling mask later on.
    ...
    # forward process, use neural network to synthesize the noise
    def p_mean_variance(
        self, model, incomplete_seq, t, noisy_context_traj, mask, noise=None
    ):
        """
        ...
        B, C = x.shape[:2]
        assert incomplete_seq.shape == (B,)
        model_output = model(x_t, self._scale_timesteps(t), noisy_context_traj=noisy_context_traj, mask=mask)
        return model_output
    ...
    def p_sample_loop(
        ...
        #      model, incomplete_seq, noisy_context_traj
        # )):
    ...
    def p_sample_loop_progressive(
        self, model, incomplete_seq, noisy_context_traj,device="cuda"
    ):
        """
        ...
        shape = incomplete_seq.shape

        # self.num_timesteps = 1000#000

        indices = list(range(self.num_timesteps))[::-1]
        # indices = list(range(10000))[::-1]
        ...
        t = torch.tensor([indices[0]] * shape[0], device=device)
        # indices = tqdm(indices)

        # mask = self.update_mask(t, incomplete_seq)
        # x_t = torch.randn_like(incomplete_seq) # X_t is the noise vector.
        # # x_t[mask==1,:] = incomplete_seq[mask==1,:]
        # x_t = torch.cat([x_t, incomplete_seq], dim=-1) 
        ############
        for i in indices:
            with torch.no_grad():
            ...
            #                                    device=model_kwargs['y'].device)
                ...
                # update mask
                # concat x_t with img               
                incomplete_seq, mask = self.create_incompelete_seq(t, incomplete_seq)
                x_t = torch.randn_like(incomplete_seq) 
                x_t = torch.cat([x_t, incomplete_seq], dim=-1) 
                # x_t = torch.cat([x_t, incomplete_seq], dim=-1) # test2 TODO:TODO: concat noise to every dimension
                x_t = model(x_t, self._scale_timesteps(t), noisy_context_traj=noisy_context_traj, mask=mask)
                # yield img
                # x_t.copy_(img)
                incomplete_seq = x_t.clone()
    ...
    # def update_mask(self, t, seq):
    #     # target TODO: rewrite'
    #     N = seq.shape[1]
    #     mask_out = torch.ones_like(seq[:,:,0])
    #     for idx, time in enumerate(t):
    #         time = self.num_timesteps - time
    #         if time >= N:
    #             continue
    ...
    def p_sample_without_grad(
        ...
        # with torch.enable_grad():
            # x = x.detach().requires_grad_()
        out = self.p_mean_variance(
            model, incomplete_seq, t, noisy_context_traj, mask, noise
        )
        noise = torch.randn_like(x)
        nonzero_mask = (
            (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
    ...
    # where finish all operation about diffusion model
    def _vb_terms_bpd(
    ...
    def training_losses(self, model, incomplete_seq, t, noisy_context_traj=None, mask=None, noise=None):
        ...
        #     model_kwargs = {}
        if noise is None:
            noise = torch.randn_like(incomplete_seq)
        x_t = self.q_sample(incomplete_seq, t, noise=noise)
        # terms = {}


        # x_t[mask==1,:] = incomplete_seq[mask==1,:] # test1 : set incomplete timestamps to noise
        ###?TODO: sep or merge? ##noise[mask==1,:] = incomplete_seq[mask==1,:] # mask the noise to get supervision | check the noise while debug | error should be diff
        x_t = torch.cat([x_t, incomplete_seq], dim=-1) # test2 TODO:TODO: concat noise to every dimension
        # TODO:TODO: don't know supervision
        ...
        return model_output,  F.mse_loss(model_output, noise, reduction='mean')# F.mse_loss(model_output[mask==0,:], noise[mask==0,:], reduction='mean')
    ...
    #     r_rot_quat, r_pos = motion_process.recover_root_rot_pos(target.permute(0, 2, 3, 1).type(torch.FloatTensor))
    ...
    #     calc_vel_from_xyz = calc_vel_from_xyz.reshape((1, 1, -1, 21, 3)).type(torch.FloatTensor)
