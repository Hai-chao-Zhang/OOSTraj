...
class Calculate():
    def __init__(self, device="cuda:0"):
        ...
        pos += "only_mse" if C.args.only_mse else ""
    ...
    def train(self, config):
        ...
        for epoch in range(args.epochs):
            ...
            for idx, (test_traj, ftm, imu, target) in enumerate(self.train_dataloader):
                ...
                if -1 in mask:
                    pred_loss = F.mse_loss(outputs, target, reduction='mean')
                    # pred_loss = F.mse_loss(outputs[mask==-1,:], target[mask==-1,:], reduction='mean') + F.mse_loss(outputs[mask==1,:], target[mask==1,:], reduction='mean')
                else:
                    pred_loss = F.mse_loss(outputs, target, reduction='mean')*2
                loss =  pred_loss if config.args.only_mse else pred_loss + diff_loss
                # , dataset=self.data.dataset)
            ...
            for idx, (test_traj, ftm, imu, target) in enumerate(self.valid_dataloader):
                ...
                sample = self.diffusion.p_sample_loop(
                    self.model,
                    incomplete_seq=test_traj, 
                    noisy_context_traj=noise_traj_context,
                    dump_steps=None
                )
                pred_loss = F.mse_loss(sample, target, reduction='mean')
                valid_epoch_loss.append(pred_loss.item())

            total_test_acc = np.average(valid_epoch_loss)
            # self.log = f"{epoch}/{args.epochs} Phase_{config.args.phase} \t {total_train_acc:.8f} \t {total_diff_train_acc:.8f} \t {total_pred_train_acc:.8f}  \t {total_test_acc:.8f} \t {total_diff_test_acc:.8f}  \t {total_test_acc:.8f}"
            self.log = f"{epoch}/{args.epochs} Phase_{config.args.phase} \t {total_train_acc:.8f} \t {total_diff_train_acc:.8f} \t {total_pred_train_acc:.8f}  \t test \t {total_test_acc*2048:.8f}"
            self.logger.info(self.log)
    ...
    def evaluate(self,config):
        # self.vifi_dataset(Dataset_ViFi)
        _, self.valid_dataloader = load_data(config)
        ...
        # valid_diff_epoch_loss = []
        valid_pred_epoch_loss = []
        self.logger.info("Test ................... Test ................... Test ................... Test\n")
        self.logger.info("Loop \t Test Loss")


        for idx, (test_traj, ftm, imu, target) in tqdm(enumerate(self.valid_dataloader),total = self.valid_dataloader.__len__()):
        ...
        total_test_acc = np.average(valid_epoch_loss)
        self.log = f" The average MSE loss is {total_test_acc:.8f} "
        self.logger.info(self.log)
        print(self.log)
        # total_diff_test_acc = np.average(valid_diff_epoch_loss)
        # total_pred_test_acc = np.average(valid_pred_epoch_loss)
...
if __name__=="__main__":
    ...
    parser.add_argument("--learning_rate", nargs="?", type=float, default= '1e-2', help="learning_rate")
    ...
    parser.add_argument("--only_mse", action='store_true', help="only_mse loss, no diffusion loss")
