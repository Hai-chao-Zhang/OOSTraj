...
class Dataset_H3D(Dataset):
    def __init__(self, flag='train'):
        ...
        #     self.seq_in_BBX5, self.seq_in_FTM2, self.gps, self.seq_out_BBX5 = prepare_training_data(self.C)
        #     if C.args.is_pos:
        #         self.gps = imu_cal(self.gps)
        #     with open('./data/indoor.pkl', 'wb') as file: # /home/haichao/code/toyota/trajpred/data/indoor.pkl
        #         # A new file will be created
        #         pickle.dump((self.seq_in_BBX5, self.seq_in_FTM2, self.gps, self.seq_out_BBX5 ), file)
        # else:
        self.seq_in_IMU19, self.seq_out_BBX5 = pickle.load(open("h3d/h3d_vifi.pkl", 'rb')) # sensor_data,gt
        self.gps = self.seq_in_IMU19[:,:,-2:]
        print("load data from pkl...")

        assert self.gps.shape[0] == self.seq_out_BBX5.shape[0]
        # assert self.seq_in_FTM2.shape[0] == self.gps.shape[0]
        ...
        def denorm(a):
            denorm = transforms.Normalize((-0.5,)*a.shape[0], (2.0,)*a.shape[0])(a)
            return denorm
        ...
        self.gps = norm(self.transform(self.gps)/64.)#.permute(1,2,0))
        self.seq_out_BBX5 = norm(self.transform(self.seq_out_BBX5)/152.)#.permute(1,2,0))

        if flag=='valid':
            # self.seq_in_BBX5 = self.seq_in_BBX5[:128]
            # self.seq_in_FTM2 = self.seq_in_FTM2[:128]
            self.gps = self.gps[:128]
            self.seq_out_BBX5 = self.seq_out_BBX5[:128]
        elif flag=='train':
            # self.seq_in_BBX5 = self.seq_in_BBX5[128:]
            # self.seq_in_FTM2 = self.seq_in_FTM2[128:]
            self.gps = self.gps[128:]
        ...
        self.length = self.seq_out_BBX5.shape[0]

    def __getitem__(self, index):
        
        # traj = self.seq_in_BBX5[index]
        # noisy_traj = torch.cat([self.seq_in_FTM2[index], self.gps[index]])
        ...
        return gps, torch.Tensor([]),  torch.Tensor([]), target
    ...
    def __load_data__(self,):
        pass
        print("ngps.shape:{}\nseq_out_BBX5.shape:{}\n"
            .format( self.gps.shape, self.seq_out_BBX5.shape))
