...
class Dataset_ViFi(Dataset):
    def __init__(self, C, flag='train'):
        ...
        self.C = C
        if C.args.is_proc_data:
            self.seq_in_BBX5, self.seq_in_FTM2, self.seq_in_IMU19, self.seq_out_BBX5 = prepare_training_data(self.C)
            # if C.args.is_pos:
            #     self.seq_in_IMU19 = imu_cal(self.seq_in_IMU19)
            # with open('./data/indoor.pkl', 'wb') as file: # /home/haichao/code/toyota/trajpred/data/indoor.pkl
            #     # A new file will be created
            #     pickle.dump((self.seq_in_BBX5, self.seq_in_FTM2, self.seq_in_IMU19, self.seq_out_BBX5 ), file)
        else:
            self.seq_in_BBX5, self.seq_in_FTM2, self.seq_in_IMU19, self.seq_out_BBX5 = pickle.load(open("./data/indoor.pkl", 'rb'))
            print("load data from pkl...")

        assert self.seq_in_BBX5.shape[0] == self.seq_out_BBX5.shape[0]
        assert self.seq_in_FTM2.shape[0] == self.seq_in_IMU19.shape[0]
        ...
        self.seq_in_BBX5 = norm(self.transform(self.seq_in_BBX5)/2048.)
        self.seq_in_FTM2 = norm(self.transform(self.seq_in_FTM2)/2048.)#.permute(1,2,0))
        self.seq_in_IMU19 = norm(self.transform(self.seq_in_IMU19)/64.)#.permute(1,2,0))
        self.seq_out_BBX5 = norm(self.transform(self.seq_out_BBX5)/2048.)#.permute(1,2,0))

        if flag=='valid':
            self.seq_in_BBX5 = self.seq_in_BBX5[:3000]
            self.seq_in_FTM2 = self.seq_in_FTM2[:3000]
            self.seq_in_IMU19 = self.seq_in_IMU19[:3000]
            self.seq_out_BBX5 = self.seq_out_BBX5[:3000]
        elif flag=='train':
            self.seq_in_BBX5 = self.seq_in_BBX5[3000:]
            self.seq_in_FTM2 = self.seq_in_FTM2[3000:]
            self.seq_in_IMU19 = self.seq_in_IMU19[3000:]
        ...
        self.length = self.seq_in_BBX5.shape[0]

    def __getitem__(self, index):
        
        traj = self.seq_in_BBX5[index]
        # noisy_traj = torch.cat([self.seq_in_FTM2[index], self.seq_in_IMU19[index]])
        ftm = self.seq_in_FTM2[index] 
        imu = self.seq_in_IMU19[index]
        target = self.seq_out_BBX5[index]

        return traj, ftm, imu, target
    ...
    def __load_data__(self,):
        pass
        print("seq_in_BBX5.shape:{}\nseq_in_FTM2.shape:{}\nseq_in_IMU19.shape:{}\nseq_out_BBX5.shape:{}\n"
            .format(self.seq_in_BBX5.shape, self.seq_in_FTM2.shape, self.seq_in_IMU19.shape, self.seq_out_BBX5.shape))
...
def prepare_training_data_for_vis(C):
    seq_in_BBX5_dfv3_ls, seq_in_FTM2_dfv3_ls, seq_in_IMU19_dfv3_ls = [], [], []
    seq_in_BBX5_dfv3_ls_mask = []
    '''
    C.BBX5_sync_dfv3 e.g. (589, 5, 5)
...
def load_data(C, datasetname=Dataset_ViFi):
    Dataset_name = datasetname
    train_dataset = Dataset_name(C, flag='train')
    train_dataloader = DataLoader(dataset=train_dataset, batch_size=128, shuffle=True)
    valid_dataset = Dataset_name(C, flag='valid')
    valid_dataloader = DataLoader(dataset=valid_dataset, batch_size=3000, shuffle=True)
